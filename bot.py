...
def get_page(title):
    r = session.get(API_URL, params={"action": "query", "prop": "revisions", "rvprop": "content|ids", "titles": title, "format": "json", "maxlag": 5})
...
def edit_page(title, text, summary):
    ...
    r = session.post(API_URL, data={"action": "edit", "title": title, "text": text, "summary": summary, "token": token, "format": "json", "maxlag": 5})
    result = r.json()
    if "error" in result:
...
def fix_duplicate_refs(text):
    ...
    changes = []
    for ref in refs:
        if ref.has("name") or is_inside_template(ref):
            continue
...
def process_item(item):
    ...
    if "title" in item:
        new_rev = edit_page(item["title"], new_text, "Bot: Convert duplicate <ref> tags to named references (bot)")
